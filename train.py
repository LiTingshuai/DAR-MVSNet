...
parser = argparse.ArgumentParser(description='official Implementation of DARMVSNet')
...
if __name__ == '__main__':
    ...
    # model, optimizer
    model = DARMVSNet(refine=False, ndepths=[int(nd) for nd in args.ndepths.split(",") if nd],
                          depth_interals_ratio=[float(d_i) for d_i in args.depth_inter_r.split(",") if d_i],
                          share_cr=args.share_cr,
                          cr_base_chs=[int(ch) for ch in args.cr_base_chs.split(",") if ch],
                          grad_method=args.grad_method)
    model.to(device)
    model_loss = dar_mvsnet_loss
